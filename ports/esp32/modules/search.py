...
def handle_search(owl, motor, ros_params):
    ...
    if motor.state == 0:  # очищаем исходные
        ...
        if motor.search_dir > 0:
            motor.target = motor.max_search
        else:
            motor.target = motor.min_search
        motor.state = 1
            ...
            if len(motor.value_best) > 0:
                if calc_angles(owl, motor):
                    if motor.angle_bisector_diff is not None:
                        motor.target = motor.angle_bisector_diff  # даем задание на перемещение в середину плато
                        motor.state = 51

                    else:
                        motor.target = motor.angle_bisector_max  # даем задание на перемещение в середину max
                        motor.state = 54
                else:
                    motor.target = motor.angle_best  # нет данных, попробуем в позиции angle_best
                    motor.state = 57
            else:
                motor.target = motor.angle_start  # даем задание на перемещение в исходную позицию
...
#                 motor.state = 12
        ...
        motor.target = motor.angle_bisector_diff
        motor.state = 52
        ...
        motor.target = motor.angle_bisector_max
        motor.state = 55
        ...
        if motor.mover.is_ready():
            ...
            if owl.mode == owl.MD_SEARCH_A:
                if motor.angle_best is None:
                    ...
                    owl.elev.target = owl.elev.angle_start + owl.elev.search_dir * owl.ANTENNA_ANGLE
                else:
