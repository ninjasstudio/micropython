...
class Stepper():
    def __init__(self, name, pin_step, pin_dir, freq=1000, reverse_direction=0):
        self.name = name
    ...
    def __repr__(self):
        return f"Stepper('{self.name}', pin_step={self.pin_step}, pin_dir={self.pin_dir}, freq={self._freq}, reverse_direction={self._reverse_direction})"
    ...
    #@micropython.native
    @steps_target.setter
    def steps_target(self, to_steps = None):
        # Set the target position that will be executed in the main loop
        # if to_steps is not None:
            self._steps_target = round(to_steps)
    ...
    def direction(self, delta:int):
        if delta > 0:
            ...
            self._direction = 0
        #print(f'{self.name} Set rotate direction:{delta} to {self._direction}')
...
#             self.direction = self._steps_target - self._steps_now
#             self.run_single_step()

#         if not self.is_ready():
#             n = (self._steps_target - self._steps_now) / 10
#             self.run_steps(n if n != 0.0 else 1 if n > 0.0 else -1)
