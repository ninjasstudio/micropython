...
def show_index_page(server, arg, owl):
    ...
    owl.s1 = "Roll:{:8.1f} Pitch:{:8.1f} Yaw:{:8.1f}".format(owl.sensors.roll, owl.sensors.pitch, owl.sensors.yaw)
    s2 = dumps(owl.value_now)
    s3 = "Азимут: " + str(round(owl.azim.mover.angle_now(), 1)) + " " + str(round(owl.azim.mover.angle_target, 1)) + " (" + str(round(owl.azim.min_search)) + ", " + str(round(owl.azim.max_search)) + ")"  # + " offset " + str(round(owl.azim.mover.offset, 1))  # + " " + str(owl.azim.mover.is_ready())  # + " " + str(owl.azim.mover.is_pulses) + " " + str(owl.azim.mover.direction)
    s4 = "Кут місця: " + str(round(owl.elev.mover.angle_now(), 1)) + " " + str(round(owl.elev.mover.angle_target, 1)) + " (" + str(round(owl.elev.min_search)) + ", " + str(round(owl.elev.max_search)) + ")"  # + " offset " + str(round(owl.elev.mover.offset, 1)) + " " + str(owl.elev.mover.is_ready())  # + " " + str(owl.elev.mover.is_pulses) + " " + str(owl.elev.mover.direction)
    ...
    if owl.mode == owl.MD_OFF:
        ...
        checked6 = "checked"  # Стеження хрестове 6 7
    ...
    s = html1.format(owl.SSID, owl.s1, s2, s3, s4)
    collect()
...
def do_CW(server, arg, owl):
    owl.azim.target = round(owl.azim.mover.angle_target + 1, 1)
    show_index_page(server, arg, owl)
    owl.auto_start = False
...
def do_UP(server, arg, owl):
    owl.elev.target = round(owl.elev.mover.angle_target + 1, 1)
    show_index_page(server, arg, owl)
    owl.auto_start = False
