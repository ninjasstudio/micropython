class Coordinate():
    def __init__(self, mover, max_search=180, min_search=-180):
        ...
        self._angle_start = 0  # стартовая позиция
    ...
    def correct_handler(self, delta_angle):
        ...
        if self.angle_start is not None:
            self.angle_start = self.angle_start + delta_angle
        if self.angle_minus is not None:
            self.angle_minus = self.angle_minus + delta_angle
        if self.angle_plus is not None:
            self.angle_plus = self.angle_plus + delta_angle
        if self.angle_best is not None:
            self.angle_best = self.angle_best + delta_angle
        if self.angle_bisector_max is not None:
            self.angle_bisector_max = self.angle_bisector_max + delta_angle
        if self.angle_bisector_diff is not None:
            self.angle_bisector_diff = self.angle_bisector_diff + delta_angle
        if self.angle_start_plus is not None:
            self.angle_start_plus = self.angle_start_plus + delta_angle
        if self.angle_start_minus is not None:
            self.angle_start_minus = self.angle_start_minus + delta_angle
...
class CoordinateShow(Coordinate):
    ...
    def angle(self, to_angle):
        self.mover.angle(to_angle)
        '''
        if self.search_dir > 0:
            print('+', self.mover.direction, self.mover.name, to_angle)
        else:
            print('-', self.mover.direction, self.mover.name, to_angle)
        '''